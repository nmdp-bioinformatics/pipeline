...
sub resultsBody{
  ...
  my %h_subject_errors;
  foreach my $s_ID (sort keys %$rh_verified){
    foreach my $s_loc (sort keys %{$$rh_verified{$s_ID}}){

      foreach my $s_typing (sort keys %{$$rh_verified{$s_ID}{$s_loc}}){
      ...
      foreach my $s_typing (sort keys %{$$rh_verified{$s_ID}{$s_loc}}){
        ...
        if(defined $$rh_qc_expected{$s_ID}{$s_loc}){
          ...
          if($$rh_qc_expected{$s_ID}{$s_loc} !~ /\|/){
            ...
            my $cnt = 1;
            print $html "<td>";
            foreach my $hap (sort keys %h_haps){
              my ($h1,$h2) = split(/\+/,$hap);
              $h1 = g2p($h1) eq g2p($s_typing) ? "<b>".$h1."</b>" : $h1;
              $h2 = g2p($h2) eq g2p($s_typing) ? "<b>".$h2."</b>" : $h2;
              $hap = join("+",$h1,$h2);
